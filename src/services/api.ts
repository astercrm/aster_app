...
getSalaryRules: () => request<any[]>('/salary-rules'),
upsertSalaryRule: (staffName: string, staffRole: string, percentage: number) =>
  request<any>('/salary-rules', { method: 'POST', body: JSON.stringify({ staffName, staffRole, percentage }) }),
updateSalaryRule: (id: string, percentage: number) =>
  request<any>(`/salary-rules/${id}`, { method: 'PUT', body: JSON.stringify({ percentage }) }),
deleteSalaryRule: (id: string) => request<void>(`/salary-rules/${id}`, { method: 'DELETE' }),
calculateSalary: (fromDate: string, toDate: string) =>
  request<any>('/salary/calculate', { method: 'POST', body: JSON.stringify({ fromDate, toDate }) }),
