...
const navItems = [
  ...
  { icon: ShoppingCart, label: 'Expenses', path: '/expenses', accountOnly: true },
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {navItems
          .filter(item => {
            if (item.adminOnly) return user?.role === 'Admin' || user?.role === 'Account'; // Admin Panel: Admin & Account
            if (item.accountOnly) return user?.role === 'Account'; // Incomes/Expenses: Account only
            return true;
          })
          .map((item) => (
            <NavLink
