...
export default function ExpensesSection({ userRole }: { userRole?: string }) {
  const isAdmin = userRole === 'Admin';
  const canManage = userRole === 'Admin' || userRole === 'Account'; // both can see product name & manage
  ...
  const [form, setForm] = useState({
    ...
    transactionId: '', billScreenshot: '', productScreenshot: '', notes: '',
  });

  const resetForm = () => {
    setForm({ date: '', productName: '', quantity: 1, amount: '', transactionId: '', billScreenshot: '', productScreenshot: '', notes: '' });
  ...
  const handleEdit = (exp: any) => {
    setForm({ date: exp.date, productName: exp.productName, quantity: exp.quantity, amount: exp.amount, transactionId: exp.transactionId, billScreenshot: exp.billScreenshot, productScreenshot: exp.productScreenshot, notes: exp.notes || '' });
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full bg-gray-50 dark:bg-slate-800 border-none rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none dark:text-white resize-none"
                />
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead><tr className="bg-gray-50 dark:bg-slate-800/50 text-[10px] uppercase tracking-wider font-bold text-gray-500 dark:text-slate-400 border-b border-gray-100 dark:border-slate-800">
              <th className="px-4 py-3">Date</th>{canManage && <th className="px-4 py-3">Product</th>}<th className="px-4 py-3">Qty</th>
              <th className="px-4 py-3">Amount</th><th className="px-4 py-3">TXN ID</th>
              <th className="px-4 py-3">Bill</th><th className="px-4 py-3">Product</th><th className="px-4 py-3">Notes</th><th className="px-4 py-3 text-right">Actions</th>
            </tr></thead>
            <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
              {loading ? (
                ...
                <tr key={exp.id} className="group hover:bg-gray-50/50 dark:hover:bg-slate-800/30">
                  ...
                  {canManage && <td className="px-4 py-3 font-bold text-gray-900 dark:text-white">{exp.productName || '—'}</td>}
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500 dark:text-slate-400 max-w-[160px] truncate">{exp.notes || '—'}</td>
