              ...
              <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10">
                  <Routes>
                    ...
                    <Route path="/admin" element={user.role === 'Admin' || user.role === 'Account' ? <AdminPanel user={user} contacts={contacts} /> : <Navigate to="/" />} />
                    <Route path="/incomes" element={user.role === 'Admin' ? <div className="space-y-6 animate-in fade-in duration-500"><header><h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Incomes</h1><p className="text-gray-500 dark:text-slate-400 mt-1">Manage and track income records.</p></header><IncomesSection contacts={contacts} /></div> : <Navigate to="/" />} />
                    <Route path="/expenses" element={user.role === 'Account' || user.role === 'Admin' ? <div className="space-y-6 animate-in fade-in duration-500"><header><h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Expenses</h1><p className="text-gray-500 dark:text-slate-400 mt-1">Manage and track expense records.</p></header><ExpensesSection userRole={user.role} /></div> : <Navigate to="/" />} />
