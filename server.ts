...
function toContact(row: any) {
  ...
  return {
    ...
    notesImages: Array.isArray(row.notes_images) ? row.notes_images : [],
    remarksImages: Array.isArray(row.remarks_images) ? row.remarks_images : [],
...
function toDB(contact: any) {
  return {
    ...
    bank_txn_id: contact.bankTxnId,
    account_notes: contact.accountNotes,
    contact_verification_status: contact.contactVerificationStatus,
    is_favorite: contact.isFavorite,
    screenshot_image: contact.screenShotImage,
    extra_images: Array.isArray(contact.extraImages) ? contact.extraImages.filter(Boolean) : [],
    notes_images: Array.isArray(contact.notesImages) ? contact.notesImages.filter(Boolean) : [],
    remarks_images: Array.isArray(contact.remarksImages) ? contact.remarksImages.filter(Boolean) : [],
...
async function startServer() {
  ...
  // ── AUTO-MIGRATE: contacts table — add new Account columns ────────────────
  const contactAlterCols = [
  ...
  for (const sql of contactAlterCols) {
  ...
  app.get('/api/contacts/ctn-list', async (req, res) => {
    const { data, error } = await supabase
      .from('contacts')
      .select('ctn, tele_calling_staff, technical_staff, customer_name, customer_contact_number, customer_requirement, current_status, details_notes, service_charges, payment_status, receive_amount, transaction_id, date, receive_date, claim_apply_date, follow_up_date, pdf_file_send, remarks, screenshot_image, extra_images')
      .not('ctn', 'is', null)
    ...
    const list = (data || [])
      ...
      .map((r: any) => ({
        ...
        customerMobile: r.customer_contact_number || '',
        customerRequirement: r.customer_requirement || '',
        currentStatus: r.current_status || '',
        detailsNotes: r.details_notes || '',
        ...
        claimApplyDate: r.claim_apply_date || '',
        followUpDate: r.follow_up_date || '',
        pdfFileSend: r.pdf_file_send || '',
        remarks: r.remarks || '',
  ...
  if (incCheck && (incCheck.code === '42P01' || incCheck.message?.includes('does not exist'))) {
    ...
    const incSQL = `CREATE TABLE IF NOT EXISTS incomes (id UUID DEFAULT gen_random_uuid() PRIMARY KEY, date TEXT, ctn_number TEXT DEFAULT '', customer_name TEXT DEFAULT '', customer_mobile TEXT DEFAULT '', staff_name TEXT, staff_role TEXT, service_charges TEXT, payment_status TEXT, receive_amount TEXT, receive_date TEXT, screenshot_image TEXT, bank_transaction_id TEXT, employee_transaction_id TEXT, is_verified BOOLEAN DEFAULT FALSE, verification_status TEXT DEFAULT '', created_at TIMESTAMPTZ DEFAULT now());`;
    await fetch(`${supabaseUrl}/rest/v1/rpc/exec_sql`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` },
    ...
    // Silently attempt to add new columns to existing table (safe — IF NOT EXISTS)
    const alterCols = [
      `ALTER TABLE incomes ADD COLUMN IF NOT EXISTS ctn_number TEXT DEFAULT '';`,
      `ALTER TABLE incomes ADD COLUMN IF NOT EXISTS customer_name TEXT DEFAULT '';`,
      `ALTER TABLE incomes ADD COLUMN IF NOT EXISTS customer_mobile TEXT DEFAULT '';`,
      `ALTER TABLE incomes ADD COLUMN IF NOT EXISTS verification_status TEXT DEFAULT '';`,
    ];
  ...
  if (expCheck && (expCheck.code === '42P01' || expCheck.message?.includes('does not exist'))) {
    ...
    const expSQL = `CREATE TABLE IF NOT EXISTS expenses (id UUID DEFAULT gen_random_uuid() PRIMARY KEY, date TEXT, product_name TEXT, quantity INT DEFAULT 1, amount TEXT, transaction_id TEXT, bill_screenshot TEXT, product_screenshot TEXT, notes TEXT DEFAULT '', created_at TIMESTAMPTZ DEFAULT now());`;
    await fetch(`${supabaseUrl}/rest/v1/rpc/exec_sql`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` },
    ...
    console.log(`  CREATE TABLE IF NOT EXISTS expenses (id UUID DEFAULT gen_random_uuid() PRIMARY KEY, date TEXT, product_name TEXT, quantity INT DEFAULT 1, amount TEXT, transaction_id TEXT, bill_screenshot TEXT, product_screenshot TEXT, notes TEXT DEFAULT '', created_at TIMESTAMPTZ DEFAULT now());`);
  } else {
    // Silently attempt to add notes column to existing expenses table (safe — IF NOT EXISTS)
    const alterExpCols = [
      `ALTER TABLE expenses ADD COLUMN IF NOT EXISTS notes TEXT DEFAULT '';`,
    ];
    for (const sql of alterExpCols) {
      await fetch(`${supabaseUrl}/rest/v1/rpc/exec_sql`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` },
        body: JSON.stringify({ query: sql }),
  ...
  app.post('/api/incomes', async (req, res) => {
    ...
    const isVerified = vs === 'manual_verified' || vs === 'auto' ? true : vs === 'manual_not_verified' ? false : (bankId !== '' && empId !== '' && bankId.toLowerCase() === empId.toLowerCase());
    const row = {
      date: b.date,
      ctn_number: b.ctnNumber || '',
    ...
    res.json({
      ...
      ctnNumber: data.ctn_number || '', customerName: data.customer_name || '', customerMobile: data.customer_mobile || '',
      staffName: data.staff_name || '', staffRole: data.staff_role || '',
      serviceCharges: data.service_charges || '', paymentStatus: data.payment_status || '',
      receiveAmount: data.receive_amount || '', receiveDate: data.receive_date || '',
      screenshotImage: data.screenshot_image || '',
      bankTransactionId: data.bank_transaction_id || '', employeeTransactionId: data.employee_transaction_id || '',
      isVerified: data.is_verified || false, verificationStatus: data.verification_status || '',
  ...
  // ── SALARY RULES (Admin only) ─────────────────────────────────────────────

  // Auto-create staff_salary_rules table if it doesn't exist
  const { error: salCheck } = await supabase.from('staff_salary_rules').select('id').limit(1);
  if (salCheck && (salCheck.code === '42P01' || salCheck.message?.includes('does not exist'))) {
    console.log('⚠️  staff_salary_rules table does not exist. Please run supabase_salary.sql in the Supabase SQL Editor.');
  }
  ...
  // POST — create or upsert a salary rule
  app.post('/api/salary-rules', async (req, res) => {
    const { staffName, staffRole, percentage } = req.body;
    if (!staffName || !staffRole) return res.status(400).json({ message: 'staffName and staffRole are required.' });
    const { data, error } = await supabase
      .from('staff_salary_rules')
      .upsert({ staff_name: staffName, staff_role: staffRole, percentage: parseFloat(percentage) || 0, updated_at: new Date().toISOString() }, { onConflict: 'staff_name,staff_role' })
      .select()
      .single();
    if (error) return res.status(500).json({ message: error.message });
    res.json({ id: data.id, staffName: data.staff_name, staffRole: data.staff_role, percentage: parseFloat(data.percentage || '0') });
  });

  // PUT — update percentage for a rule by id
  app.put('/api/salary-rules/:id', async (req, res) => {
    const { percentage } = req.body;
    const { data, error } = await supabase
      .from('staff_salary_rules')
      .update({ percentage: parseFloat(percentage) || 0, updated_at: new Date().toISOString() })
      .eq('id', req.params.id)
      .select()
      .single();
    if (error) return res.status(500).json({ message: error.message });
  ...
  // DELETE — remove a salary rule
  app.delete('/api/salary-rules/:id', async (req, res) => {
    const { error } = await supabase.from('staff_salary_rules').delete().eq('id', req.params.id);
    if (error) return res.status(500).json({ message: error.message });
    res.json({ success: true });
  });

  // POST /api/salary/calculate — compute salary for a given period
  // Body: { fromDate: 'YYYY-MM-DD', toDate: 'YYYY-MM-DD' }
  // Returns per-staff breakdown: contacts count, receive total, percentage, salary amount
  // ──────────────────────────────────────────────────────────────────────────────
  // IMPORTANT: Staff names come from the Admin "Manage Dropdown" lists
  // (teleCallingStaff / technicalStaff). Contacts' staff values are matched
  // against these canonical dropdown names so there are NEVER duplicates.
  // ──────────────────────────────────────────────────────────────────────────────
  app.post('/api/salary/calculate', async (req, res) => {
    ...
    // ── Helper: strip a string down to a lowercase, space-free key for comparison ──
    ...
    // ── 1. Fetch canonical staff names from dropdown_options ──────────────────
    const { data: ddRows } = await supabase
      .from('dropdown_options')
      .select('category, label')
      .in('category', ['teleCallingStaff', 'technicalStaff'])
      .order('sort_order', { ascending: true });

    // Build lookup: key → canonical dropdown label
    const teleDropdownNames: string[] = [];
    const techDropdownNames: string[] = [];
    const teleKeyToCanonical: Record<string, string> = {};
    const techKeyToCanonical: Record<string, string> = {};

    (ddRows || []).forEach((row: any) => {
      const key = toKey(row.label);
      if (row.category === 'teleCallingStaff') {
        teleDropdownNames.push(row.label);
        teleKeyToCanonical[key] = row.label;
      } else {
        techDropdownNames.push(row.label);
        techKeyToCanonical[key] = row.label;
      }
    });

    // ── Helper: match a contact's staff value to the canonical dropdown name ──
    // Tries exact key match first; falls back to "contains" match for prefixed
    // names like "D_Ramya" matching dropdown "Ramya".
    function matchTele(raw: string): string | null {
      const r = (raw || '').trim();
      if (!r) return null;
      const key = toKey(r);
      // Exact key match
      if (teleKeyToCanonical[key]) return teleKeyToCanonical[key];
      // Partial / contains match — check if any dropdown name key is contained
      // in the raw key or vice-versa (handles "D Ramya" matching "Ramya")
      for (const [dk, canonical] of Object.entries(teleKeyToCanonical)) {
        if (key.includes(dk) || dk.includes(key)) return canonical;
      }
      return null; // no match → skip
    }
    function matchTech(raw: string): string | null {
      const r = (raw || '').trim();
      if (!r) return null;
      const key = toKey(r);
      if (techKeyToCanonical[key]) return techKeyToCanonical[key];
      for (const [dk, canonical] of Object.entries(techKeyToCanonical)) {
        if (key.includes(dk) || dk.includes(key)) return canonical;
      }
      return null;
    }

    // ── 2. Fetch contacts + salary rules ─────────────────────────────────────
    const { data: contacts, error: cErr } = await supabase
      .from('contacts')
      .select('tele_calling_staff, technical_staff, receive_amount, receive_date, date')
      .not('receive_amount', 'is', null);
    if (cErr) return res.status(500).json({ message: cErr.message });

    const { data: rules } = await supabase.from('staff_salary_rules').select('*');
    const ruleMap: Record<string, number> = {};
    (rules || []).forEach((r: any) => {
      // Store rule under both original and key-based lookups
      ruleMap[`${r.staff_name}::${r.staff_role}`] = parseFloat(r.percentage || '0');
      // Also store under the canonical dropdown name if we can find it
      const key = toKey(r.staff_name);
      if (r.staff_role === 'TeleCalling' && teleKeyToCanonical[key]) {
        ruleMap[`${teleKeyToCanonical[key]}::TeleCalling`] = parseFloat(r.percentage || '0');
      }
      if (r.staff_role === 'Technical' && techKeyToCanonical[key]) {
        ruleMap[`${techKeyToCanonical[key]}::Technical`] = parseFloat(r.percentage || '0');
      }
    });
    ...
    function parseContactDate(d: string): Date | null {
      if (!d) return null;
    ...
    // ── 4. Group contacts by CANONICAL dropdown staff name ───────────────────
    // Only names that match the dropdown list appear. No duplicates possible.
    const teleMap: Record<string, { count: number; total: number }> = {};
    const techMap: Record<string, { count: number; total: number }> = {};

    (contacts || []).forEach((c: any) => {
      const amt = parseFloat(c.receive_amount || '0') || 0;
      if (amt <= 0) return;
      const rd = parseContactDate(c.receive_date) || parseContactDate(c.date);
      if (!rd || rd < from || rd > to) return;

      // Map contact staff name → canonical dropdown name
      const tele = matchTele(c.tele_calling_staff);
      const tech = matchTech(c.technical_staff);

      if (tele) {
        if (!teleMap[tele]) teleMap[tele] = { count: 0, total: 0 };
        teleMap[tele].count++;
        teleMap[tele].total += amt;
      }
      if (tech) {
        if (!techMap[tech]) techMap[tech] = { count: 0, total: 0 };
        techMap[tech].count++;
        techMap[tech].total += amt;
      }
    });

    // ── 5. Build results using canonical names ──────────────────────────────
    const teleResults = Object.entries(teleMap).map(([name, data]) => {
      const pct = ruleMap[`${name}::TeleCalling`] ?? 0;
      return { staffName: name, staffRole: 'TeleCalling', contactCount: data.count, receiveTotal: data.total, percentage: pct, salaryAmount: (data.total * pct) / 100 };
    }).sort((a, b) => a.staffName.localeCompare(b.staffName));

    const techResults = Object.entries(techMap).map(([name, data]) => {
      const pct = ruleMap[`${name}::Technical`] ?? 0;
      return { staffName: name, staffRole: 'Technical', contactCount: data.count, receiveTotal: data.total, percentage: pct, salaryAmount: (data.total * pct) / 100 };
    }).sort((a, b) => a.staffName.localeCompare(b.staffName));

    res.json({ tele: teleResults, technical: techResults, fromDate, toDate });
  });
